...
def get_args():
    ...
    parser.add_argument(
        "--model",
        # default='02_model/MobileNetV1/SingleHandLocalization_1.0_128.onnx')
        default='02_model/EfficientNetB0/SingleHandLocalization_224.onnx')
...
def main():
    ...
    while True:
        ...
        predict_result = onnx_session.run([output_localization, output_class],
                                          {input_name: x})

        class_id = np.argmax(np.squeeze(predict_result[0]))
        class_score = np.squeeze(predict_result[0])[class_id]
        point = np.squeeze(np.squeeze(predict_result[1]))
        ...
        # 画面反映 #############################################################
        cv.imshow('onnx inference sample', debug_image)
